ALTER TABLE public.insumo_purchase_entries
  ...
  ALTER COLUMN data_compra DROP DEFAULT,
  ADD COLUMN IF NOT EXISTS quantidade_embalagens NUMERIC,
  ADD COLUMN IF NOT EXISTS conteudo_por_embalagem NUMERIC;

DROP FUNCTION IF EXISTS public.register_insumo_entry(UUID, NUMERIC, NUMERIC, DATE, UUID);
...
REVOKE ALL ON FUNCTION public.register_insumo_entry(UUID, NUMERIC, NUMERIC, DATE, UUID, NUMERIC, NUMERIC) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.register_insumo_entry(UUID, NUMERIC, NUMERIC, DATE, UUID, NUMERIC, NUMERIC) TO authenticated;
