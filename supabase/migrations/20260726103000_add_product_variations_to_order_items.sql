...
CREATE INDEX IF NOT EXISTS idx_itens_pedido_produto_variacao_id
  ON public.itens_pedido (produto_variacao_id);

UPDATE public.itens_pedido ip
SET
  produto_id = COALESCE(ip.produto_id, b.produto_id),
  produto_variacao_id = COALESCE(ip.produto_variacao_id, b.produto_variacao_id)
FROM public.brigadeiros b
WHERE ip.brigadeiro_id = b.id
  AND (ip.produto_id IS NULL OR ip.produto_variacao_id IS NULL);
