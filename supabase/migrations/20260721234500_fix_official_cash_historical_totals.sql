-- Corrige separação entre caixa oficial e histórico comercial.
-- O caixa oficial só considera transações oficiais a partir de 01/08/2026.
-- Entradas anteriores ao marco ficam no histórico comercial sem duplicar pedidos
-- históricos já entregues até 31/07/2026.
...
BEGIN
  ...
  RETURN QUERY
  WITH historical_delivered_orders AS (
    SELECT p.id
    FROM public.pedidos p
  ...
  historical_pre_start_entries AS (
    SELECT t.valor::numeric AS valor
    FROM public.transacoes t
    LEFT JOIN historical_delivered_orders hdo
      ON t.referencia LIKE 'pedido:' || hdo.id::TEXT || ':%'
  ...
  official_tx AS (
    ...
    FROM public.transacoes t
    WHERE t.user_id = v_user_id
      AND public.is_official_financial_transaction(t.user_id, t.referencia, t.data)
  )
  SELECT
    ...
    (
      ...
      + COALESCE((SELECT SUM(valor) FROM historical_pre_start_entries), 0)
