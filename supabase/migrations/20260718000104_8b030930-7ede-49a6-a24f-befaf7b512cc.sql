...
ALTER TABLE public.pedidos
  ...
  DROP CONSTRAINT IF EXISTS pedidos_entrega_endereco_check,
...
BEGIN
  ...
  VALUES (
    ...
    NULLIF(btrim(COALESCE(p_endereco_entrega, '')), ''), p_canal_venda,
    p_valor_total, COALESCE(p_valor_pago, 0), p_forma_pagamento,
    v_status_operacional, v_status_operacional, v_status_financeiro,
  ...
  SELECT v_pedido.id, NULLIF(item->>'brigadeiro_id', '')::UUID, item->>'brigadeiro_nome',
  ...
  IF v_pedido.valor_pago > 0 THEN
    ...
    VALUES (v_user_id, 'pedido', v_pedido.id, 'payment_created',
...
BEGIN
  ...
  IF v_delta = 0 THEN RETURN v_pedido; END IF;

  UPDATE public.pedidos
     SET valor_pago = p_valor_pago, status_financeiro = v_status_financeiro,
  ...
  INSERT INTO public.transacoes (user_id, tipo, categoria, descricao, valor, data, referencia)
  VALUES (v_user_id, 'entrada', 'Vendas', 'Pagamento - Pedido ' || v_pedido.cliente, v_delta, CURRENT_DATE, v_ref);

  INSERT INTO public.audit_log (user_id, entity_type, entity_id, action, metadata)
  VALUES (v_user_id, 'pedido', p_pedido_id, 'payment_created',
    jsonb_build_object('from', v_old_valor_pago, 'to', p_valor_pago, 'delta', v_delta, 'referencia', v_ref));
