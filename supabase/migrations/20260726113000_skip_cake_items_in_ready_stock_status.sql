...
BEGIN
  ...
  IF p_status IN ('pronto', 'entregue') AND v_old_status <> 'entregue' THEN
    FOR v_item IN
      ...
      LEFT JOIN public.brigadeiros b ON b.id = ip.brigadeiro_id AND b.user_id = v_user_id
      LEFT JOIN public.produtos p ON p.id = ip.produto_id AND p.user_id = v_user_id
      WHERE ip.pedido_id = p_pedido_id
        AND ip.quantidade > 0
        AND COALESCE(p.categoria_codigo, b.categoria, 'brigadeiro') = 'brigadeiro'
      GROUP BY ip.brigadeiro_id, ip.brigadeiro_nome
    LOOP
      ...
      LEFT JOIN public.brigadeiros b ON b.id = ip.brigadeiro_id AND b.user_id = v_user_id
      LEFT JOIN public.produtos prod ON prod.id = ip.produto_id AND prod.user_id = v_user_id
      WHERE p.user_id = v_user_id
        AND p.id <> p_pedido_id
        AND p.archived_at IS NULL
        AND p.status_operacional = 'pronto'
        AND ip.quantidade > 0
        AND COALESCE(prod.categoria_codigo, b.categoria, 'brigadeiro') = 'brigadeiro'
        AND (
