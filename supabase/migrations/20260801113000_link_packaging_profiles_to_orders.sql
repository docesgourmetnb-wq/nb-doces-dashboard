ALTER TABLE public.pedidos
  ADD COLUMN IF NOT EXISTS packaging_profile_id UUID REFERENCES public.packaging_profiles(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS packaging_profile_nome TEXT;

CREATE INDEX IF NOT EXISTS idx_pedidos_packaging_profile_id
  ON public.pedidos (packaging_profile_id);

DROP FUNCTION IF EXISTS public.create_pedido_with_items(TEXT, UUID, DATE, TEXT, NUMERIC, TEXT, TEXT, TEXT, JSONB, DATE, TEXT, TEXT, TEXT, NUMERIC);
...
BEGIN
  ...
  IF p_packaging_profile_id IS NOT NULL THEN
    SELECT pp.nome
    INTO v_packaging_profile_nome
    FROM public.packaging_profiles pp
    WHERE pp.id = p_packaging_profile_id
  ...
  VALUES (
    ...
    p_packaging_profile_id, v_packaging_profile_nome
  )
