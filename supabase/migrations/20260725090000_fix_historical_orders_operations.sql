CREATE OR REPLACE FUNCTION public.is_historical_order(p_data_entrega date, p_data date)
RETURNS boolean
LANGUAGE sql
STABLE
SET search_path TO 'public'
AS $$
  SELECT COALESCE(p_data_entrega, p_data) < DATE '2026-08-01';
$$;
...
BEGIN
  ...
  SELECT
    ...
    COALESCE((SELECT SUM(valor) FROM historical_orders), 0)::numeric AS total_historico
  FROM official_tx;
END;
$$;
...
BEGIN
  ...
  itens_entregues AS (
    SELECT
      ...
      b.categoria AS produto_categoria,
      b.tamanho_g AS produto_tamanho_g
    FROM delivered_period p
    JOIN public.itens_pedido i ON i.pedido_id = p.id
    LEFT JOIN public.brigadeiros b ON b.id = i.brigadeiro_id AND b.user_id = v_user_id
  ),
  produtos_rank AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object('nome', nome, 'tamanho', tamanho, 'quantidade', quantidade, 'receita', receita) ORDER BY receita DESC), '[]'::jsonb) AS data
    FROM (
      SELECT
        COALESCE(NULLIF(regexp_replace(COALESCE(MAX(produto_nome), MAX(brigadeiro_nome)), '\s+\d+(?:[,.]\d+)?\s*g$', '', 'i'), ''), MAX(brigadeiro_nome)) AS nome,
        CASE
          WHEN MAX(produto_categoria) = 'brigadeiro' AND MAX(produto_tamanho_g) IS NOT NULL
            THEN replace(trim(to_char(MAX(produto_tamanho_g), 'FM999999990.999')), '.', ',') || 'g'
          ELSE substring(COALESCE(MAX(produto_nome), MAX(brigadeiro_nome)) from '(\d+(?:[,.]\d+)?)\s*g$')
        END AS tamanho,
        COALESCE(SUM(quantidade), 0)::int AS quantidade,
        COALESCE(SUM(quantidade * preco_unitario), 0)::numeric AS receita
      FROM itens_entregues
      GROUP BY COALESCE(brigadeiro_id::text, lower(brigadeiro_nome))
  ...
  sabores_rank AS (
    ...
    FROM (
      SELECT
        ...
        COALESCE(SUM(quantidade), 0)::int AS quantidade
      FROM itens_entregues
      GROUP BY lower(COALESCE(NULLIF(regexp_replace(COALESCE(produto_nome, brigadeiro_nome), '\s+\d+(?:[,.]\d+)?\s*g$', '', 'i'), ''), brigadeiro_nome))
...
REVOKE ALL ON FUNCTION public.is_historical_order(date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_historical_order(date, date) TO authenticated;
